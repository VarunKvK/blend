...
import { Download, Code, Sparkles, AlertCircle, X, CheckCircle2 } from 'lucide-react';
import { extractColorsFromImage, generateCssGradient, generateSvgContent } from './utils/colorUtils';
import { GradientConfig, UserTier, ExportFormat, Point } from './types';
import { DIMENSION_PRESETS, DEFAULT_GRADIENT_COLORS, MAX_DAILY_EXPORTS, LAYOUTS } from './constants';
...
function App() {
  ...
  const [userTier, setUserTier] = useState<UserTier>({ type: 'FREE', dailyExportsLeft: MAX_DAILY_EXPORTS });
  ...
  const handlePaletteChange = (newColors: string[]) => {
    setGradientConfig(prev => {
        // If color count changes, re-generate points based on layout
  ...
  const handleShuffle = useCallback(() => {
    if (gradientConfig.type === 'mesh') {
        // Shuffle points but keep layout logic? Or fully random?
        // Let's shuffle the colors mapping to the points
  ...
  }, [gradientConfig.colors, gradientConfig.type]);

  const handleMeshPointUpdate = (index: number, x: number, y: number) => {
    setGradientConfig(prev => {
    ...
    if (format === ExportFormat.CSS) {
        ...
        if (gradientConfig.type === 'mesh' && gradientConfig.meshPoints) {
            ...
            gradientConfig.colors.forEach((color, i) => {
                ...
                const r = Math.min(width, height) * 0.5; // Large radius for overlap
        ...
        if (gradientConfig.noise > 0) {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                // Apply grain in an overlay/additive way
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-black text-zinc-100 overflow-hidden font-sans">
      ...
      <div className="w-full md:w-[380px] flex-shrink-0 bg-black border-r border-white/10 flex flex-col h-screen z-20 shadow-2xl">
        ...
        <div className="p-6 space-y-8 flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-800">
            ...
            <div>
                ...
                <div className="relative">
                    <select 
                        className="w-full bg-zinc-900 border border-zinc-800 text-white text-sm rounded-md focus:ring-1 focus:ring-white focus:border-white block p-2.5 appearance-none"
        ...
        <div className="p-6 border-t border-white/10 bg-black">
            <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <button 
                    onClick={() => handleExport(ExportFormat.CSS)}
            ...
            {userTier.type === 'FREE' && (
                <div className="flex items-center justify-between text-[10px] text-zinc-600 px-1 uppercase tracking-widest">
                    ...
                    <button onClick={() => setShowPaywall(true)} className="text-white hover:underline">
                        Go Pro
                    </button>
                </div>
            )}
      ...
      <div className="flex-1 relative h-[50vh] md:h-screen bg-[#050505]">
        <PreviewCanvas 
            ...
            onUpdateMeshPoints={handleMeshPointUpdate}
        />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-fade-in">
            <div className="bg-zinc-950 border border-zinc-800 rounded-lg max-w-sm w-full p-8 relative">
                ...
                <button 
                    ...
                    className="w-full py-3 bg-white text-black font-bold rounded-md hover:bg-zinc-200 transition-colors"
                >
                    Unlock for $9
                </button>
            </div>
